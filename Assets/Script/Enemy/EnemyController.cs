...
public class EnemyController : MonoBehaviour, IHittable, IStateMachineOwner, IAttackEventListener, IAttackClipSource, IJumpEventListener
{
    [Header("공격 대상 레이어")]
    [KoreanLabel("대상 레이어")]
    ...
    void TryStartAttack()
    {
        // 점프 준비 동작/착지 경직 중엔 공격을 무시한다. 여기서 공격이 시작되면 해당 클립이
    ...
    /// Airborne(공중 넉백) > Stun(그라운드 슬라이드) > Attack > InAir(공중) > JumpStart(준비) > JumpLand(착지) > Move/Idle.
    /// </summary>
